...
#GOALS looks the same as TURN requests
        for each_da in current_DA:
            if each_da in informable_slots:
                ...
                #    tf.print ("IS CONFIRM, BUT GOES AS REQUEST: ", each_da) 
        ...
        current_information_richness = []
        #request, food, location, price
#         j/mentions/#slots/richness
#        |1|0|4|0|  case: no mention
#        |2|0|4|0|
#        |3|0|4|0|
#        |4|0|4|0|
#
#        |1|1|4|1|  case: 1 mention
#        |2|1|4|0|
#        |3|1|4|0|
#        |4|1|4|0|
#
#        |1|2|4|0|  case: 2 mentions
#        |2|2|4|1|
#        |3|2|4|0|
#        |4|2|4|0|
#
#        |1|3|4|0|  case: 3 mentions
#        |2|3|4|0|
#        |3|3|4|1|
#        |4|3|4|0|
#
#        |1|4|4|0|  case: 4 mentions
#        |2|4|4|0|
#        |3|4|4|0|
#        |4|4|4|1|
#
#        |1|5|4|0|  case: 5 mentions
#        |2|5|4|0|
#        |3|5|4|0|
#        |4|5|4|1|

        for j in range(1, 4+1):
            if j == slots_mentioned or j == 4 and 4 < slots_mentioned:
                current_information_richness.append(1)
            else:
...
def generate_data(utterances, word_vectors, dialogue_ontology):
    """
    ...
    list_of_slots = list(dialogue_ontology.keys()) #["food", "area", "price range", "request"]
...
def generate_npa_examples(feature_vectors, word_vectors, dialogue_ontology,
                      utterances_train, positive_indices):
    """
    ...
    for utterance_idx in positive_indices:
        ...
        delex_features = numpy.zeros_like(utterance[0][0])

        features_full.append(utterance_fv[0])
        features_masks.append(utterance_fv[1])
        features_requested_slots.append(utterance_fv[2])
        features_confirm_slots.append(utterance_fv[3])
        features_confirm_values.append(utterance_fv[4])
...
def generate_examples_predict(target_slot, feature_vectors, word_vectors, dialogue_ontology,
    ...
    return (features_full, features_masks, features_requested_slots, features_confirm_slots, \
            features_confirm_values, features_delex, features_previous_state)
...
def extract_feature_vectors(utterances, word_vectors, ngram_size=3, longest_utterance_length=40, use_asr=False, use_transcription_in_training=False, num_actions=70):
    ...
    for idx, utterance in enumerate(utterances):
        ...
        for sub_idx in range(0, confirmation_count):
            ...
            if current_cslot != "" and current_cvalue != "":
                if " " not in current_cslot:
                    current_conf_slot_vector += word_vectors[str(current_cslot)]
                else:
        ...
        #mask by max asr, not for each asr
        max_asr_length = 0
...
def train_run(target_language, override_en_ontology, percentage, model_type, dataset_name, word_vectors, exp_name, dialogue_ontology, model_variables, target_slot, language="en", max_epoch=20, batches_per_epoch=4096, batch_size=256):
    ...
    slots = list(dialogue_ontology.keys())
...
def print_slot_predictions(distribution, slot_values, target_slot, threshold=0.05):
    ...
    tf.print("Predictions for", str(target_slot + ":"), predicted_values)
...
class NeuralBeliefTracker:
    ...
    def __init__(self, config_filepath):
        ...
        word_vectors["<unk>"] = xavier_vector("<unk>")

        print (word_vectors["<unk>"])

        word_vectors["</s>"] = xavier_vector("</s>")

        print (word_vectors["</s>"])

        word_vectors = {word_vectors, load_word_vectors(word_vector_destination, primary_language=language)}
    ...
    if do_training:
        ...
        while True:
            ...
            if req_slot not in NBT.dialogue_ontology:
                print(req_slot, "---", list(NBT.dialogue_ontology.keys()))
                req_slot = ""
            else:
                req_slot = req_slot
